...
public:
    ...
    void query(const string &s) { // 拓扑排序
        for (int i = 0, u = 0; i < s.length(); ++i)
            u = son[u][s[i] - 'a'], /* add tag */;
        queue<int> q;
        for (int i = 0; i <= tot; ++i)
            if (!deg[i])
                q.push(i);
    ...
    void query(const string &s) { // 暴力遍历
        for (int i = 0, u = 0; i < s.length(); ++i) {
            u = son[u][s[i] - 'a'];
            for (int j = lst[u]; j; j = lst[fail[j]])
                // do something
        }
    }
};
...
若文本串长度为 $n$，模式串总长度为 $m$，则模式串的长度种数为 $O(\sqrt m)$，对于每一种长度，对应在文本串中只有 $O(n)$ 个位置，因此暴力遍历的复杂度为 $O(n \sqrt m)$。
...
class PAM {
private:
    int lst, tot, len[N], fail[N], son[N][C];
    string s;

public:
    void clear() { lst = tot = 1, len[1] = -1, fail[0] = 1; }
    ...
    void insert(char c) {
        s.push_back(c);
        lst = find(lst, s.length() - 1);
        if (!son[lst][c - 'a'])
            fail[++tot] = son[find(fail[lst], s.length() - 1)][c - 'a'], len[tot] = len[lst] + 2, son[lst][c - 'a'] = tot;
        lst = son[lst][c - 'a'];
    }
};
```
...
得到一组 $ax + by = \gcd(a, b)$ 的解。

```cpp
void exgcd(int a, int b, int &x, int &y) {
    if (!b)
        return x = 1, y = 0, a;
    exgcd(b, a % b, y, x), y -= x * (a / b); // 大部分情况下这里都需要取模
}
```
...
int exgcd(int a, int b, int &x, int &y) {
    ...
    return y = dec(y, mul(x, a / b)), d;
}
int inv(int a) {
    int x, y;
    return exgcd(a, mod, x, y) != 1 ? -1 : x;
}
```
